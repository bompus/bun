...
impl Watcher {
    ...
    pub(crate) fn dispatch_file_updates(&mut self, event_count: usize, changed_count: usize) {
        ...
        // A kqueue NOTE_RENAME / NOTE_DELETE on a directory's own vnode means
        // its path no longer names that vnode. Remember it, so that when the
        // path exists again `remove_replaced_descendants` treats the directory
        // as replaced even if the same inode was moved back. (inotify reports
        // a child's deletion on the directory watch with the same op, so this
        // is kqueue-only; inotify names the entry that reappears instead.)
        #[cfg(any(target_os = "macos", target_os = "freebsd"))]
    ...
    /// Queues eviction (see `flush_evictions`) of every watchlist entry whose
    /// path is `dir_path` or lies below it, and reports each one to `each`.
    ///
    /// For when the directory at `dir_path` was replaced: renamed over
    /// (`mv lib lib.old && mv lib.new lib`), or removed and created again.
    /// inotify and kqueue watch inodes, not paths, so every registration at or
    /// below that path now observes the old directory tree (or nothing), and
    /// `add_file` / `add_directory` would keep matching the stale entries by
    /// path hash instead of watching the new ones. Evicting them lets the next
    /// module load re-arm fresh watches.
    ///
    /// Same contract as `remove_at_index::<false>`: the caller holds
    /// `self.mutex` (it runs inside `on_file_update`). Returns the number of
    /// entries queued.
    pub fn remove_path_and_descendants(
        &mut self,
        dir_path: &[u8],
    ...
    /// changed, with no names. Finds every watched directory below it whose
    /// path now names a different inode than the one its fd holds (or names
    /// anything again once its vnode was `displaced`), and evicts that subtree
    /// through `remove_path_and_descendants`. A directory whose path does not
    /// exist at the moment is left alone: the parent receives another write
    /// event when something reappears under that name.
    #[cfg(not(windows))]
    pub fn remove_replaced_descendants(
        &mut self,
        ...
        for i in 0..slice.len() {
            ...
            #[cfg(any(target_os = "macos", target_os = "freebsd"))]
            let displaced = slice.items_displaced()[i];
            #[cfg(not(any(target_os = "macos", target_os = "freebsd")))]
            let displaced = false;
            if !displaced {
