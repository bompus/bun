    ...
    let blob = {
        ...
        match DataURL::parse_without_check(url_utf8.slice())
            ...
            None => {
                ...
                return JSPromise::rejected_promise_value(global_this, err);
            }
        }
    };
...
fn reject_on_exception(
    ...
    Ok(JSPromise::rejected_promise_value(global_this, err))
}
...
fn fetch_impl<const ALLOW_GET_BODY: bool>(
    ...
    if let Some(sig) = &signal {
        if sig.aborted() {
            ...
            body.detach();
            return Ok(JSPromise::rejected_promise_value(global_this, reason));
        }
    }
    ...
    if body.is_s3() {
        'prepare_body: {
            ...
            let rejected_value = JSPromise::rejected_promise_value(
                global_this,
        ...
        'prepare_body: {
            ...
            let opened_fd = match opened_fd_res {
                Err(err) => {
                    ...
                    let rejected_value = JSPromise::rejected_promise_value(global_this, err_js);
                    return Ok(rejected_value);
                }
                Ok(fd) => fd,
            };
