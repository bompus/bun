...
describe.concurrent(() => {
  ...
  it("emits rejectionHandled for a late catch no matter how the promise was rejected", async () => {
    ...
    // .finally() passthrough, a native stream erroring) used to get
    // 'unhandledRejection' but never 'rejectionHandled', because reading the
    // rejection reason in order to report it marked the promise as handled.
    await using proc = Bun.spawn({
      cmd: [
        bunExe(),
        "-e",
        `
          ...
          const kinds = {
            ...
            erroredBody: () => new Response(new ReadableStream({ start(c) { c.error(new Error("erroredBody")); } })).text(),
          };
