// Temporary probe, part 5 of 7: more rounds of the parallel batch in which
...
test.skipIf(!isLinux || isMusl || process.arch !== "arm64")(
  "linker-order.test.ts inside the parallel batch that hung, part 5",
  async () => {
    const { rounds, bad } = await runHungBatchRounds("part5", 150_000);
    for (const out of bad) console.error(`=== stalled batch output (filtered)\n${out}`);
    console.log(`part 5: ${rounds} rounds, ${bad.length} with a tracer stall`);
