// Temporary probe, part 4 of 7: more rounds of the parallel batch in which
...
test.skipIf(!isLinux || isMusl || process.arch !== "arm64")(
  "linker-order.test.ts inside the parallel batch that hung, part 4",
  async () => {
    const { rounds, bad } = await runHungBatchRounds("part4", 150_000);
    for (const out of bad) console.error(`=== stalled batch output (filtered)\n${out}`);
    console.log(`part 4: ${rounds} rounds, ${bad.length} with a tracer stall`);
