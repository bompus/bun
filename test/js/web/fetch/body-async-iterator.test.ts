...
// The generator must not run ahead of a JS reader: Bun pulls one chunk past what the reader
// took, then waits for the next read.
for (const [label, make] of [
  ["Response", gen => new Response(gen)],
  ["Request", gen => new Request("http://localhost/", { method: "POST", body: gen, duplex: "half" })],
] as const) {
