...
When the stream is read from JavaScript (`getReader()`, `pipeTo()`, `for await`), the buffer is full once `highWaterMark` bytes wait for the reader. The default is 64 KiB. Pass `{ highWaterMark }` as the second `ReadableStream` constructor argument to change it.
